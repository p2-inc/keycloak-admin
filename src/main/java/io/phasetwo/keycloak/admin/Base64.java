 ...
 *
 * <p>Example: <code>String encoded = Base64.encode( myByteArray );</code> <br>
 ...
 * call like this: <code>
 * String encoded = Base64.encodeBytes( mybytes, Base64.GZIP | Base64.DO_BREAK_LINES );</code>
 *
 * <p>to compress the data before encoding it and then making the output have newline characters.
 *
 * <p>Also... <code>String encoded = Base64.encodeBytes( crazyString.getBytes() );</code>
 *
 * <p>Change Log:
 *
 * <ul>
 *   <li>v2.3.8 - Fixed automatic gzip decoding, based on the content, as this may lead to
 *       unexpected behaviour. Request either gzipped or non gzipped decoding as excepted. Automatic
 *       encoding is especially problematic with generated input (see KEYCLOAK-18914 for a detailed
 *       case).
 ...
 *       characters in it.
 *   <li>v2.3.6 - Fixed bug when breaking lines and the final byte of the encoded string ended in
 *       the last column; the buffer was not properly shrunk and contained an extra (null) byte that
 *       made it into the string.
 *   <li>v2.3.5 - Fixed bug in {@link #encodeFromFile} where estimated buffer size was wrong for
 ...
 *             possibility of an java.io.IOException being thrown. After some discussion and
 *             thought, I've changed the behavior of the methods to throw java.io.IOExceptions
 *             rather than return null if ever there's an error. I think this is more appropriate,
 *             though it will require some changes to your code. Sorry, it should have been done
 *             this way to begin with.
 *         <li><em>Removed all references to System.out, System.err, and the like.</em> Shame on me.
 *             All I can say is sorry they were ever there.
 *         <li><em>Throws NullPointerExceptions and IllegalArgumentExceptions</em> as needed such as
 *             when passed arrays are null or offsets are invalid.
 *         <li>Cleaned up as much javadoc as I could to avoid any javadoc warnings. This was
...
public class Base64 {
   ...
   * very least should not be called Base64 without also specifying that it was encoded using the
   ...
   * does not check to make sure your arrays are large enough to accommodate <var>srcOffset</var> +
   * 3 for the <var>source</var> array or <var>destOffset</var> + 4 for the <var>destination</var>
   ...
   *
   * <p>As of v 2.3, if there is an error with the GZIP stream, the method will throw an
   ...
   * Encodes a byte array into Base64 notation. Does not GZip-compress data.
   *
   * <p>As of v 2.3, if there is an error, the method will throw an java.io.IOException. <b>This is
   * new to v2.3!</b> In earlier versions, it just returned a null value, but in retrospect that's a
   ...
   * along their length by specifying <var>srcOffset</var> and <var>destOffset</var>. This method
   * does not check to make sure your arrays are large enough to accommodate <var>srcOffset</var> +
   * 4 for the <var>source</var> array or <var>destOffset</var> + 3 for the <var>destination</var>
   ...
   * @return the decoded data
   * @throws java.io.IOException if there is an error (invalid character in source string or gunzip
   *     error)
